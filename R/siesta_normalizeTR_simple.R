siesta_normalizeTR_simple <- function(data, limits = list("low" = c(42, 0.8), "high" = c(60, 0.3))) {
  ...
  message("Proteins for selection of normalization model: ", nrow(good_proteins))
  ...
  lowPass <- s_data %>%
    ...
    group_by( id) %>%
    summarise(minVal = min(Value, na.rm=T))
  numSamples <- length(unique(s_data$Sample))
  highPass <- s_data %>%
    filter(Temperature >= limits$high[1]) %>%
    group_by(id) %>%
    summarise(maxVal = max(Value, na.rm=T))
